(() => {
    ...
    function initSliders() {
        if (document.querySelector(".team__body")) new Swiper(".team__body", {
            ...
            watchOverflow: false,
            navigation: {
                prevEl: ".arrows-team__button--prev",
                nextEl: ".arrows-team__button--next"
            },
            pagination: {
                el: ".team__pagination",
                clickable: true,
                bulletClass: "pagination-team__dot",
                bulletActiveClass: "pagination-team__dot--active"
            },
            breakpoints: {
                768: {
                    slidesPerView: 2,
    ...
    if (themeSwitch) {
        const themeImage = themeSwitch.querySelector("img");
        const updateThemeUI = isDark => {
            if (isDark) {
                document.body.classList.add("dark-theme");
                if (themeImage) themeImage.src = themeImage.src.replace("theme-switch.svg", "theme-switch-active.svg");
            } else {
                document.body.classList.remove("dark-theme");
                if (themeImage) themeImage.src = themeImage.src.replace("theme-switch-active.svg", "theme-switch.svg");
            }
        };
        const savedTheme = localStorage.getItem("theme");
        let isDark = false;
        if (savedTheme) isDark = savedTheme === "dark"; else if (document.body.classList.contains("news-detail-page") || document.body.classList.contains("news-page")) isDark = true;
    ...
    const copyArticleBtn = document.querySelector(".copy-article-btn");
    if (copyArticleBtn) copyArticleBtn.addEventListener("click", () => {
        const url = window.location.href;
        navigator.clipboard.writeText(url).then(() => {
            const originalText = copyArticleBtn.innerText;
            copyArticleBtn.innerText = "Ссылка скопирована!";
            copyArticleBtn.classList.add("copied");
            setTimeout(() => {
                copyArticleBtn.innerText = originalText;
